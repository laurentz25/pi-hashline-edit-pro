...
1. **Hash length: 2 → 4 characters.** Combined with the alphabet change below, this gives 24 bits of entropy per anchor (16 777 216 buckets), up from 8 bits (256 buckets) in upstream.
...
- `HASH_LENGTH` in `src/hashline/hash.ts` is the single source of truth. It is referenced by every regex, every parse check, every error message, and every test fixture.
...
- `src/` contains the implementation, split by responsibility: `read.ts`, `edit.ts`, `edit-normalize.ts`, `edit-diff.ts`, `edit-response.ts`, `edit-render.ts`, `file-kind.ts`, `fs-write.ts`, `utils.ts`, and small runtime/path helpers. The hashline engine is in `src/hashline/` with sub-modules: `hash.ts` (hash computation), `parse.ts` (anchor parsing), `resolve.ts` (validation and resolution), `apply.ts` (edit application), and `index.ts` (re-exports).
...
- If you bump `HASH_LENGTH` to 5, update every test that constructs a 4-char fixture (grep for `[A-Za-z0-9_\-]{4}`, `toHaveLength(4)`, and the test bodies in `test/core/hashline.parse.test.ts` and `test/core/hashline.resolve.test.ts`). The test suite is the contract for the wire format.
...
The bare-prefix detector (`HASHLINE_BARE_PREFIX_RE`) rejects any edit line whose first 5 characters (after optional leading whitespace) are 4 alphabet chars followed by `:`. With a 64-char alphabet, the regex `^\s*[A-Za-z0-9_-]{4}:` matches a LOT of code (4-char identifiers like `init:`, `data:`, `from:`, `Type:` followed by `:`). The trade-off: the detector may reject legitimate code that happens to use a 4-char identifier + colon. We accept the false-positive cost because the model can rephrase (quote, escape, change shape) and retry, while a false negative leaves a stray hash in the file. The error code `[E_BARE_HASH_PREFIX]` is distinct from `[E_INVALID_PATCH]` (which still catches the unambiguous `+HASH:` and `-N   ` diff-row forms at the parse stage). When a suspect's prefix happens to match a real file-line hash, the error message calls that out — strong evidence the model was reading a `HASH:content` line and copied only the prefix.

## Tool output token-efficiency

The hash prefix is paid on every `read` line, and on every `--- Updated anchors ---` block in `edit` results. 4-character hashes cost two more characters of overhead per line than the upstream's 2-character hashes — that adds up on large files but is the price of the collision resistance.
