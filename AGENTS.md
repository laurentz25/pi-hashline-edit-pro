...
1. **Hash format: `#` prefix + 4 characters.** Combined with the alphabet change below, this gives 24 bits of entropy per anchor (16 777 216 buckets), up from 8 bits (256 buckets) in upstream. The `#` prefix makes anchors unambiguous and eliminates false positives from common code patterns.
...
- `HASH_LENGTH` in `src/hashline/hash.ts` is the single source of truth for the hash body length (4 chars). `HASH_PREFIX` (`#`) is the prefix marker. `ANCHOR_LENGTH` (prefix + body = 5) is the total wire-format length.
...
- If you bump `HASH_LENGTH`, update every test that constructs a fixture hash (grep for `toHaveLength(5)`, `#[A-Za-z0-9_\-]{4}`, and the test bodies in `test/core/hashline.parse.test.ts` and `test/core/hashline.resolve.test.ts`). The test suite is the contract for the wire format.
...
The bare-prefix detector (`HASHLINE_BARE_PREFIX_RE`) rejects any edit line whose first 6 characters (after optional leading whitespace) are `#` + 4 alphabet chars + `:`. With the `#` prefix, this regex is highly specific and eliminates false positives from common code patterns like `init:`, `data:`, `else:`, etc. The error code `[E_BARE_HASH_PREFIX]` is distinct from `[E_INVALID_PATCH]` (which still catches the unambiguous `+#HASH:` and `-N   ` diff-row forms at the parse stage). When a suspect's prefix happens to match a real file-line anchor, the error message calls that out — strong evidence the model was reading a `#HASH:content` line and copied only the prefix.

## Tool output token-efficiency

The hash prefix (`#`) is paid on every `read` line, and on every `--- Updated anchors ---` block in `edit` results. 5-character anchors (`#` + 4 hash chars) cost three more characters of overhead per line than the upstream's 2-character hashes — that adds up on large files but is the price of the collision resistance and unambiguous format.
