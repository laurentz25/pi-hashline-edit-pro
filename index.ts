...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerCommand("read", {
    description:
      "Read a file with hashline anchors and add to chat (usage: /read @<path>)",
    handler: async (args, ctx) => {
      ...
      const displayPath = isAbsolute(rawPath) ? rawPath : absolutePath;
      ...
      if (file.kind === "directory") {
        ctx.ui.notify(`Path is a directory: ${displayPath}`, "error");
        return;
      }
      if (file.kind === "binary") {
        ctx.ui.notify(
          `Binary file: ${displayPath} (${file.description})`,
          "error",
        );
        return;
      }
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    try {
      ...
      const displayLines = truncated
        ? visibleLines.slice(0, MAX_LINES)
        : visibleLines;
