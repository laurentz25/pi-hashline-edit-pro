...
- All edits in one call must be non-conflicting. The runtime rejects with `[E_EDIT_CONFLICT]` if two `replace` ranges overlap. Fix: merge into one or split into a follow-up `edit` call.
...
The legacy `oldText`/`newText` shape (top-level) is rejected with `[E_LEGACY_SHAPE]`. Use hash-anchored edits instead.
